...
namespace Services
{
    public class SceneControllerBase : Service
    {
        ...
        public void LoadScene(string name, bool needConfirm = false, LoadSceneMode mode = LoadSceneMode.Single)
            => core.LoadScene(SceneControllerUtility.ToSceneIndex(name), mode, async, needConfirm);

        public void LoadScene(int index, bool needConfirm = false, LoadSceneMode mode = LoadSceneMode.Single)
            => core.LoadScene(index, mode, async, needConfirm);
        public void LoadNextScene(bool needConfirm = false, LoadSceneMode mode = LoadSceneMode.Single)
            => LoadScene(SceneControllerUtility.SceneIndex + 1, needConfirm, mode);
